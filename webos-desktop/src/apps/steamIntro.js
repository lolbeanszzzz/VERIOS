...
const STEAM_STEPS = [
  ...
  },
  {
    ...
    body: "Spend coins on profile borders, titles, and reaction flair that travels with you across VERIOS.",
...
export async function offerSteamTour() {
  ...
  const accept = await os.dialog.confirm(
    "Tour VERI Steam",
    "VERI Steam packs a store, library, daily quests, a profile store, and a live community. Want a quick tour?"
  );
  if (isAnyTourActive()) return;
  if (accept) {
